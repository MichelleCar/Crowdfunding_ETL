...
* One clear and overriding conclusion that can be drawn from this exercise is that when designing and building an ETL system, it is important to have a clear perspective and plan (end goal). It's easy to start transferring data from a specific source and immediately populate tables that can be queried. However, if there is no clear plan or vision that addresses scalability, long-term management of the data, nor a clear sense of purpose for the end user, then the whole exercise is pointless.
